...
void radixSort(int *array, int size) {
    ...
    CUDA_CHECK(cudaMalloc((void **)&inArrayShared, sizeof(int) * BLOCKSIZE));
    CUDA_CHECK(cudaMalloc((void **)&outArrayShared, sizeof(int) * RADIX));
    CUDA_CHECK(cudaMalloc((void **)&radixArrayShared, sizeof(int) * BLOCKSIZE));
    ...
    printf("CUDA Time = %.5f ms GIPS = %d MAX_DIGIT = %d BLOCKSIZE = %d dim=%d\n", transferTime, GIPS, MAX_DIGIT, BLOCKSIZE, size);
